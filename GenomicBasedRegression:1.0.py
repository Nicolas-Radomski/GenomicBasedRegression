...
# USAGE: python3.12 GenomicBasedRegression:1.0.py training -m genomic-profils-for-training.tsv -p phenotypes.tsv -o MyDirectory -x FirstAnalysis -s 80 -d 4
# USAGE: python3.12 GenomicBasedRegression:1.0.py prediction -m genomic-profils-for-prediction.tsv -l MyDirectory/FirstAnalysis_labels.obj -e MyDirectory/FirstAnalysis_encoded_categories.obj -t MyDirectory/FirstAnalysis_model.sav -o MyDirectory -x SecondAnalysis -d 4
...
parser = ap.ArgumentParser(
	...
	description='Perform linear regression-based training or prediction from categorical genomic data',
	epilog=reference
	)
...
parser_training = subparsers.add_parser('training', help='Help of the model training')
...
parser_training.add_argument(
	...
	action='store', 
	required=True, 
	help='path of tab-separated values (tsv) file including profiles of mutations (REQUIRED)'
	)
parser_training.add_argument(
	'-p', '--phenotypes', 
	...
	action='store', 
	required=True, 
	help='path of tab-separated values (tsv) file including profiles of phenotypes (REQUIRED)'
	)
parser_training.add_argument(
	'-s', '--split', 
	dest='splitting', 
	...
	required=False, 
	default=80, 
	help='percentage of splitting to prepare the training dataset (DEFAULT: 80)'
	)
parser_training.add_argument(
	'-o', '--output', 
	dest='outputpath', 
...
parser_prediction.add_argument(
	'-l', '--labels', 
	dest='inputpath_labels', 
	action='store', 
	required=True, 
	help='path of object (obj) file including trained sci-kit learn labels (REQUIRED)'
	)
parser_prediction.add_argument(
	'-e', '--encoded', 
	dest='inputpath_encoded_categories', 
	action='store', 
	required=True, 
	help='path of object (obj) file including trained sci-kit learn encoded categories (REQUIRED)'
	)
parser_prediction.add_argument(
	'-t', '--model', 
	...
	action='store', 
	required=True, 
	help='path of saved (sav) file including a trained sci-kit learn model (REQUIRED)'
	)
parser_prediction.add_argument(
	'-o', '--output', 
	dest='outputpath', 
...
if args.subcommand == 'training':
	...
	INPUTPATH_MUTATIONS=args.inputpath_mutations
	INPUTPATH_LABELS=args.inputpath_labels
	INPUTPATH_ENCODED_CATEGORIES=args.inputpath_encoded_categories
...
if NOCHECK == False :
	...
	if pi.format_version != "4.0":
		...
		exit()
	if sk.__version__ != "1.6.1":
		raise Exception('sklearn 1.6.1 version is recommended')
		exit()
...
if args.subcommand == 'training':
	...
	## save the encoded categories
	encoded_categories = encoder.categories_

	# encode categorical data into binary data for the testing dataset
	## instantiate encoder object
	encoder = OneHotEncoder(handle_unknown = 'ignore', sparse_output = False, categories = encoded_categories).set_output(transform='pandas')
	# transform data with the encoder into an array for the testing dataset using produced categories
	X_test_encoded = encoder.fit_transform(X_test[X_train.columns])
	...
	outpath_labels = OUTPUTPATH + '/' + PREFIX + '_labels' + '.obj'
	outpath_encoded_categories = OUTPUTPATH + '/' + PREFIX + '_encoded_categories' + '.obj'
	outpath_model = OUTPUTPATH + '/' + PREFIX + '_model' + '.sav'
	...
	pi.dump(labels, open(outpath_labels, 'wb'))
	## save the encoded_categories
	pi.dump(encoded_categories, open(outpath_encoded_categories, 'wb'))
	## save the model
	...
	log_file.writelines(outpath_labels + "\n")
	log_file.writelines(outpath_encoded_categories + "\n")
	...
	df_mutations = pd.read_csv(INPUTPATH_MUTATIONS, sep='\t')
	## labels
	labels =  pi.load(open(INPUTPATH_LABELS, 'rb'))
	## encoded categories
	encoded_categories = pi.load(open(INPUTPATH_ENCODED_CATEGORIES, 'rb'))
	## model
	...
	encoder = OneHotEncoder(handle_unknown = 'ignore', sparse_output = False, categories = encoded_categories).set_output(transform='pandas')
	# transform data with the encoder into an array for the testing dataset using produced categories
	X_mutations_encoded = encoder.fit_transform(X_mutations[labels])
